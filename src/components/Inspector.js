...
import * as Babel from "../bindings/Babel.js";
...
import Babel$1 from "insight-kit/lib/babel";
...
function babelTranspile(script, filename, runId) {
  try {
    var transformResult = Standalone.transform(script, {
          filename: filename,
          plugins: [
            [
    ...
    var transformResult_code = transformResult.code.replace(new RegExp("export ", "g"), "");
    var transformResult_map = transformResult.map;
    var transformResult_ast = transformResult.ast;
    var transformResult$1 = {
      code: transformResult_code,
      map: transformResult_map,
      ast: transformResult_ast
    };
    return {
            TAG: 0,
            _0: transformResult$1,
            [Symbol.for("name")]: "Ok"
          };
  }
  catch (raw_err){
    var err = Caml_js_exceptions.internalToOCamlException(raw_err);
    console.warn("Exception while hyperevaling", err);
    return {
            TAG: 1,
            _0: err,
            [Symbol.for("name")]: "Error"
...
function babelInvocations(schema, trace, chain, requestValueCache) {
  return Belt_Array.map(chain.requests, (function (request) {
                ...
                var simplifiedPayload = Js_dict.fromArray(Belt_Array.map(Js_dict.entries(payload), (function (param) {
                            return [
                                    param[0],
                                    param[1].graphQLResult
                                  ];
...
function evalBabelInQuick(transformResult, insight, onSuccess, onError) {
  var store = insight.store;
  var evalResults = Babel.Insight.asyncHyperEval({
        TAG: 0,
        _0: transformResult,
        [Symbol.for("name")]: "Ok"
      }, Babel.Insight.wasmQuickJSRunner);
  if (evalResults.TAG !== /* Ok */0) {
    return Curry._1(onError, evalResults._0);
  }
  evalResults._0.then(function (results) {
        Belt_Array.forEach(results, (function (record) {
                store.addRecord(record);
                
              }));
        return Promise.resolve(Curry._2(onSuccess, results, store));
      });
