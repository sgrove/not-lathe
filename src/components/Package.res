...
module Publish = {
  ...
  let packageVersion = (~package) => {
    let (major, minor, patch) = package.version
    j`${major->string_of_int}.${minor->string_of_int}.${patch->string_of_int}`
  }

  let packageDotJson = (~package: t) => {
    let version = packageVersion(~package)
    {
      "name": package.name,
      "description": package.description->Belt.Option.getWithDefault(
        "A package generated by OneGraph Studio",
      ),
      "version": version,
  ...
  let packageFiles = (~schema, ~config: Config.Studio.t, ~chains) => {
    let (indexJs, indexDTs) =
      chains
      ->Belt.Array.map(((docId, chain)) => {
        let typeDef = Chain.typeScriptDefinition(~schema, chain)
        let js =
          chain->Chain.TypeScript.fetchSource(
            ~schema,
            ~appId=config.oneGraphAppId,
            ~docId,
            ~typeScriptDefinition=typeDef,
          )
      ...
      ->Belt.Array.reduce(("", ""), ((indexJs, indexDTs), {source, types}) => {
        (indexJs ++ "\n\n" ++ source, indexDTs ++ "\n\n" ++ types)
      })

    let indexJs = j`import fetchPonyfill from 'fetch-ponyfill';
...
${indexJs}`
    ...
    Debug.assignToWindowForDeveloperDebug(~name="prettify", prettify)

    let indexJs = try {
      indexJs->prettify
    } catch {
    | err =>
      Js.Console.error2("Error prettifying index.js: ", err)
      indexJs
    }
    ...
    | err =>
      Js.Console.error2("Error prettifying index.d.ts: ", err)
      indexDTs
    }
  ...
  let makePackage = (~schema, ~config: Config.Studio.t, ~package: t, ~chains) => {
    let json = packageDotJson(~package)
    let files = packageFiles(~schema, ~config, ~chains)
    {
      "packageDotJson": json,
      "files": files,
    }
                  ...
                  <tr
                    ...
                    <td className="py-3 px-6 ">
                      <div className="relative text-lg bg-transparent text-gray-800">
                        <div className="flex items-center ml-2 mr-2">
                          <input
                            defaultValue={Publish.packageVersion(~package)}
                            style={ReactDOMStyle.make(~backgroundColor=Comps.colors["gray-9"], ())}
                            className="border-none px-2 leading-tight outline-none text-white form-input"
                            type_="text"
                            ...
                            onChange={event => {
                              ...
                              let version = switch value->Js.String2.split(".") {
                              | [major, minor, patch] =>
                                try {
                                  Some((
                                    major->int_of_string,
                              ...
                              version->Belt.Option.forEach(version => {
                                let newPackage = {...package, version: version}
                                onEditPackage(newPackage)
                              })
                            }}
                          />
  ...
  | Package =>
    <PackageEditor
      ...
      onPublishPackageToNpm={(~npmAuth) => {
        ...
        persistedChainPromises->Js.Promise.all->Js.Promise.then_(persistedChains => {
          ...
          let npmPackage = Publish.makePackage(
            ~schema,
            ~config,
          ...
          OneGraphRe.publishNpmPackage(
            ...
            ~variables={
              ...
              "files": npmPackage["files"]->Obj.magic,
              "name": state.package.name,
              "description": state.package.description,
              "version": npmPackage["packageDotJson"]["version"],
              "dependencies": Obj.magic(npmPackage["packageDotJson"]["dependencies"]),
              "registry": #NPM,
            },
          )
        }, _)->Js.Promise.then_(_ => {
          Utils.alert("Published!")->Js.Promise.resolve
        }, _)->ignore
      }}
      onPublishPackageToGitHub={(~gitHubOAuthToken) => {
        let persistedChainPromises = state.package.chains->Belt.Array.map(chain => {
          let promise = Js.Promise.make((~resolve, ~reject as _) => {
            ChainEditor.persistChain(
              ~schema,
        ...
        persistedChainPromises->Js.Promise.all->Js.Promise.then_(persistedChains => {
          ...
          OneGraphRe.publishNpmPackage(
            ...
            ~variables={
              "npmAuth": None,
              "gitHubOAuthToken": Some(gitHubOAuthToken),
              "files": npmPackage["files"]->Obj.magic,
              "name": state.package.name,
              "description": state.package.description,
              "version": npmPackage["packageDotJson"]["version"],
